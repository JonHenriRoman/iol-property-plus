...
Parses ``fixtures/feed.xml`` (the real feed's XML shape, contact details replaced
with synthetic values), imports into a throwaway schema, re-runs to show zero
duplicates, then re-runs against the feed with one listing removed to show the
soft-delete. No network, no credentials.
...
def _feed_without(reference: str) -> str:
    """Rebuild the fixture without the ``<Property>`` block for ``reference``."""
    ...
    trimmed = head + "".join(kept)
    tmp = Path(tempfile.mkdtemp()) / "feed_removed_one.xml"
...
def main() -> int:
    with scratch_schema() as db:
        ...
        _pass(db, "pass 3 (listing 3001 removed from feed)", _feed_without("3001"))
        ...
        with db.connect() as conn:
            ...
            agencies = conn.execute("SELECT count(*) AS n FROM agencies").fetchone()["n"]
    ...
    print(f"\nlistings: {listings}   listing_media: {media}")
    print(f"agencies: {agencies}   agents: {agents}")

...
Parses ``fixtures/feed.xml`` with the streaming XML parser, imports into a
throwaway schema, re-runs to show zero duplicates, then re-runs against the feed
with one listing removed to show the soft-delete. No network, no key.
...
def _feed_without(reference: str) -> str:
    ...
    tmp = Path(tempfile.mkdtemp()) / "feed.xml"
...
def main() -> int:
    with scratch_schema() as db:
        ...
        _pass(db, "pass 3 (1597 removed from feed)", _feed_without("1597"))
    ...
    print(
        f"\nlistings: {listings}   listing_media: {media}   agents: {agents}   agencies: {agencies}"
    )
